...
async function loadVideos() {
  try {
    ...
    console.log('📹 Attempting to load videos from Supabase...');
    ...
    try {
      ...
      if (error) {
        console.warn('⚠️ Supabase Error (401):', error.message);
        console.log('ℹ️ This is normal if the videos table doesn\'t exist yet');
        ...
        console.log('📺 No videos in Supabase, using sample data');
        shouldUseSampleData = true;
      }
    } catch (supabaseError) {
      console.warn('⚠️ Supabase Connection Error:', supabaseError.message);
      console.log('ℹ️ Using sample data. To setup real data: Run VIDEOS_SCHEMA.sql in Supabase SQL Editor');
      shouldUseSampleData = true;
    }
    ...
    console.log(`✨ Ready to display ${allVideos.length} videos`);
